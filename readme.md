
Demo  video : https://youtu.be/SzyxsjOSkFg

## 📂 Estructura del Repositorio

```text
/
├── README.md                # Descripción general y guía del proyecto
├── /wireframes              # Prototipos en papel con anotaciones técnicas
│   ├── home.jpg
│   ├── plp.jpg
│   ├── pdp.jpg
│   ├── cart.jpg
│   └── checkout.jpg
├── /design                  # Entregables de alta fidelidad
│   ├── ecommerce_v1.fig      # Archivo fuente de Figma
│   └── specs_access.pdf      # Exportación con especificaciones de diseño
├── /docs                    # Documentación técnica y de ingeniería
│   ├── decisiones.md         # Racional de diseño vinculado a SWEBOK
│   ├── diagramas.puml        # Código fuente de diagramas (PlantUML)
│   └── guia-estilos.md       # Guía de estilo (Colores, Tipografía, UI)
└── /demo                    # Video demostrativo

    └── demo_flujo.mp4        # Explicación de navegación y accesibilidad

